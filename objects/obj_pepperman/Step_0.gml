...
if ((sprite_index == chargespr or sprite_index == hurtspr) && flashing == 0)
{
    alarm[6] = 1;
    flashing = 1;
}
...
if (obj_player.state == states.grab or obj_player.state == states.superslam or !place_meeting(x, y + 1, obj_collisionparent))
{
    stunned = 1;
    movespeed = 0;
    charging = 0;
...
if (thrown == 0 && (obj_player.state == states.grabbing or obj_player.state == states.grab or obj_player.state == states.Throw or obj_player.state == states.punch or obj_player.state == states.uppunch or obj_player.state == states.superslam or obj_player.state == states.shoulder or obj_player.state == states.backkick))
{
    charging = 0;
    chargingdown = 0;
    movespeed = 0;
...
//did alarms in gms1 not need a number higher than 1.00 to activate??? orignal value: 0.01
	alarm[3] = 0.02 * room_speed;
